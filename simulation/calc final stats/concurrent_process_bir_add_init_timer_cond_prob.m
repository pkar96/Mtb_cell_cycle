...
% generations over a the population. Initiation is sizer. division is
% concurrent process model with a adder from birth and timer from
...
function [tds, vbs, vds, vios, cds, rts, moth_infos] = concurrent_process_bir_add_init_timer_cond_prob(params)
...
sigcd= params(5)*num(num_in,4); % std dev of time additive division noise
...
ii_in = bii+ randn()*cvt;
if rand()>prob
    while (ii_in<x.vNextDiv_dv || ii_in<x.tNextDiv_in*x.rate+x.v)
        ii_in=bii+ randn()*cvt;
    end
else
    while ii_in<x.v
...
if rand()>prob
    while (ii_in_1<x.vNextDiv_dv || ii_in_1<x.tNextDiv_in*x.rate+x.v)
        ii_in_1=bii+ randn()*cvt;
    end
else
    while ii_in_1<x.v
...
while simTime < gens
    ...
    if  ~isempty(ind)
        ...
        elseif length(x.vNextInit)==2
            ...
            for i=1:length(ind)
                ...
                if length(ind)==2
                    ii_in = bii+ randn()*cvt;
                    while ii_in<bii/2 || ii_in<(x.vNextDiv_dv) || ii_in<(x.v+x.tNextDiv_in(1)*x.rate) || ii_in*2<(x.v+x.tNextDiv_in(end)*x.rate)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1<bii/2 || ii_in_1<(x.vNextDiv_dv) || ii_in_1<(x.v+x.tNextDiv_in(1)*x.rate) || ii_in_1*2<(x.v+x.tNextDiv_in(end)*x.rate)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                elseif length(ind)==1
                    ii_in = bii+ randn()*cvt;
                    while ii_in*2<x.vNextInit(1) || ii_in<(x.vNextDiv_dv) || ii_in<(x.v+x.tNextDiv_in(1)*x.rate) || ii_in*2<(x.v+x.tNextDiv_in(end)*x.rate)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1*2<x.vNextInit(1) || ii_in_1<(x.vNextDiv_dv) || ii_in_1<(x.v+x.tNextDiv_in(1)*x.rate) || ii_in_1*2<(x.v+x.tNextDiv_in(end)*x.rate)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                end
    ...
    if ~isempty(x.tNextDiv_in) && x.tNextDiv_in(1)<=0 && x.v - x.vNextDiv_dv >=0
