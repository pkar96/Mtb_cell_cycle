...
% generations over a single lineage unlike the population data which
% was simulated by him. The noise in initiation and division is time
% additive. Use this.

function [tds, vbs, vds, vios, cds, rts, moth_infos] = concurrent_process_bir_add_init_timer(params)
...
ngen=14; % number of generation
cd= params(1);% C+D period
bii= params(2); % 2*num(num_in,23); %5.3 change in volume from initiation to initiation
...
ii_in = bii+ randn()*cvt;
while ii_in<0 % [Changed here]
    ii_in=bii+ randn()*cvt;
end
ii_in_1 = bii+ randn()*cvt;
while ii_in_1<0 % [Changed here]
    ii_in_1=bii+ randn()*cvt;
end
...
while simTime < gens
    ...
    if  ~isempty(ind)
        ...
        elseif length(x.vNextInit)==2
            ...
            for i=1:length(ind)
                if length(ind)==2
                    ii_in = bii+ randn()*cvt;
                    while ii_in<bii/2 || ii_in<x.vNextDiv_dv || ii_in<(x.v+x.tNextDiv_in(1)*x.rate)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1<bii/2 || ii_in_1<x.vNextDiv_dv || ii_in_1<(x.v+x.tNextDiv_in(1)*x.rate)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                elseif length(ind)==1
                    ii_in = bii+ randn()*cvt;
                    while ii_in*2<x.vNextInit(1) || ii_in<x.vNextDiv_dv || ii_in<(x.v+x.tNextDiv_in(1)*x.rate)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1*2<x.vNextInit(1) || ii_in_1<x.vNextDiv_dv || ii_in_1<(x.v+x.tNextDiv_in(1)*x.rate)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                end
    ...
    if ~isempty(x.tNextDiv_in) && x.tNextDiv_in(1)<=0 && x.v - x.vNextDiv_dv >=0
        ...
        rts=[rts; 1/tds(end)*(vds(end)-vbs(end))];
