...
% timer from birth and adder from initiation. More stringent conditions on
% initiation length i.e., it can happen after one of the concurrent 
% processes (from initiation) is complete.

function [tds, vbs, vds, vios, cds, rts, moth_infos] = init_sizer_bir_tim_init_add_cond_init(params)
cells=[];
...
bii= params(2); %2*num(num_in,23); % change in volume from initiation to initiation
tbd= params(3); % 14;% change in volume from birth to division
...
cvtbd= params(6)*num(num_in,1); % std dev of vol additive division noise
cvr= num(num_in, 36); % std dev in r
cve=num(num_in,6); % std dev of experimental noise

cntr= 0; % counts number if times division is limiting
tcnt= 0;
...
while simTime < gens
    ...
    if  ~isempty(ind)
        ...
        if length(x.vNextInit)==1
            ...
            ii_in = bii+ randn()*cvt;
            while ii_in<x.vNextInit || ii_in<x.vNextDiv_in
                ii_in=bii+ randn()*cvt;
            end
            ii_in_1 = bii+ randn()*cvt;
            while ii_in_1<x.vNextInit || ii_in_1<x.vNextDiv_in
                ii_in_1=bii+ randn()*cvt;
            end
            x.vNextInit = [x.oris(1)*(ii_in) x.oris(2)*(ii_in_1)];
        ...
        elseif length(x.vNextInit)==2
            ...
            for i=1:length(ind)
                ...
                if length(ind)==2
                    ii_in = bii+ randn()*cvt;
                    while ii_in<bii/2 || ii_in<(x.v+x.tNextDiv_dv*x.rate) || ii_in<x.vNextDiv_in(1) || ii_in*2<x.vNextDiv_in(end)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1<bii/2 || ii_in_1<(x.v+x.tNextDiv_dv*x.rate) || ii_in_1<x.vNextDiv_in(1) || ii_in_1*2<x.vNextDiv_in(end)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                elseif length(ind)==1
                    ii_in = bii+ randn()*cvt;
                    while ii_in*2<x.vNextInit(1) || ii_in<(x.v+x.tNextDiv_dv*x.rate) || ii_in<x.vNextDiv_in(1) || ii_in*2<x.vNextDiv_in(end)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1*2<x.vNextInit(1) || ii_in_1<(x.v+x.tNextDiv_dv*x.rate) || ii_in_1<x.vNextDiv_in(1) || ii_in_1*2<x.vNextDiv_in(end)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                end
    ...
    %-----Divide
    %Upon accumulating enough volume
    if ~isempty(x.vNextDiv_in) && x.v-x.vNextDiv_in(1)>=0 && x.tNextDiv_dv <=0

        % find no of cases in which division is limiting 
        if x.tNextDiv_dv>=-tStep
            cntr=cntr+1;
        end
        tcnt=tcnt+1;
