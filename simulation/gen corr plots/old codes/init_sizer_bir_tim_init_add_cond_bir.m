...
% processes (from birth) is complete.Closer to actual

function [tds, vbs, vds, vios, cds, rts, moth_infos] = init_sizer_bir_tim_init_add_cond_bir(params)
cells=[];
    ...
    if  ~isempty(ind)
        ...
        elseif length(x.vNextInit)==2
            ...
            for i=1:length(ind)
                if length(ind)==2
                    ii_in = bii+ randn()*cvt;
                    while ii_in<bii/2 || ii_in<(x.v+x.tNextDiv_dv*x.rate) || ii_in<x.vNextDiv_in(1)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1<bii/2 || ii_in_1<(x.v+x.tNextDiv_dv*x.rate) || ii_in_1<x.vNextDiv_in(1)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                elseif length(ind)==1
                    ii_in = bii+ randn()*cvt;
                    while ii_in*2<x.vNextInit(1) || ii_in<(x.v+x.tNextDiv_dv*x.rate) || ii_in<x.vNextDiv_in(1)
                        ii_in=bii+ randn()*cvt;
                    end
                    ii_in_1 = bii+ randn()*cvt;
                    while ii_in_1*2<x.vNextInit(1) || ii_in_1<(x.v+x.tNextDiv_dv*x.rate) || ii_in_1<x.vNextDiv_in(1)
                        ii_in_1=bii+ randn()*cvt;
                    end
                    x.vNextInit = [x.vNextInit 2*(ii_in) 2*(ii_in_1)];
                end
